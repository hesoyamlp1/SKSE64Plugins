#!/usr/bin/env python3
# Vendored from https://gist.github.com/mmozeiko/7f3162ec2988e81e56d5c4e22cde9977
# (mmozeiko, "portable-msvc.py") — downloads a standalone MSVC + Windows SDK
# toolchain into ./msvc without installing anything system-wide.
# Used by build.sh when no Visual Studio / Build Tools installation is found.
# Only modification: first() is annotated -> Any so static type checkers don't
# flag the many call sites that immediately subscript its result.
...
def first(items, cond = lambda x: True) -> Any:
...
for target in targets:
  if target not in ALL_TARGETS:
    sys.exit(f"Unknown target architecture!")
...
try:
  ...
  if isinstance(err.args[0], ssl.SSLCertVerificationError):
    # for more info on Python & issues with Windows certificates see https://stackoverflow.com/a/52074591
    print("ERROR: ssl certificate verification error")
    try:
      import certifi
    except ModuleNotFoundError:
      print("ERROR: please install 'certifi' package to get Mozilla certificates")
...
for arch in ["x86", "x64", "arm", "arm64"]:
  ...
  if arch != host:
    shutil.rmtree(OUTPUT / "VC/Tools/MSVC" / f"bin/Host{arch}", ignore_errors=True)
